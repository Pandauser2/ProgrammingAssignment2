...
StudentVle %>%filter(id_student==633902) %>% arrange(desc(sum_click))
 ...
 # only one record with over 1000 clicks; probably logging issue
...
rename(StudentAssessment,date= final_assessment_submission_date)
...
# 5. Join Student click aggregated activity to studentinfo; left join since all student may not have an activity

StudentInfo <- merge(StudentInfo, Studentclicks_agg, by = c("code_module","code_presentation","id_student"), all.x= TRUE)


# join Student info with StudentAssessment_agg, left join since student may not submitted assessment
...
colnames(StudentInfo)[17] = "total_material_accessed"

colnames(StudentInfo)[18]="total_times_accessed"

colnames(StudentInfo)[16]="average_clicks_agg"
